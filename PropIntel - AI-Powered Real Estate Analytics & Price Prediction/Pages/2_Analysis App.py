...
for col in numeric_columns:
    new_df[col] = pd.to_numeric(new_df[col], errors='coerce')  # Convert, setting invalid to NaN
...
if isinstance(feature_text, list):
    feature_text = ' '.join(feature_text)  # Convert list to a single string
elif isinstance(feature_text, dict):
    feature_text = ' '.join(feature_text.values())  # Merge dictionary values into a string

# Generate WordCloud
wordcloud = WordCloud(
    width=800, height=800,
    background_color='black',
    stopwords=set(['s', 'the', 'and', 'to', 'of', 'in']),  # Add meaningful stopwords
    min_font_size=10
